\chapter{Katamaran}
Katamaran is a framework for verifying \textmu{}Sail programs using \glspl{uc}. \sand{While we focus on veryfing UCs in the case studies, Katamaran is not limited to "just that". It supports veryfication of any kind.}
A \gls{uc} specifies the expected behaviour of a program in terms of preconditions and postconditions, and Katamaran proves that the given \textmu{}Sail implementation satisfies these contracts. \sand{In the case of security-properties, the UC captures the security guarantees of the ISA. Make sure you also explain why and what UCs are. They allow reasoning about (the interaction with) unknown code as well. If all you have is known code, than you wouldn't need a UC.}
...
Katamaran includes a case study for the RISC-V \gls{pmp} extension. \sand{nit: for RISC-V *with* the PMP extension. Current phrasing seems to single out just PMP, which would also be possible}
...
The existing model is however intentionally limited to serve as a proof of concept. \sand{The model is limited due to the manual translation we did at the time and scaling reasons. It's not a proof of concept, it's a real system with a real verification.}
It only supports two \gls{pmp} entries, rather than 16 or 64 allowed in the real RISC-V specification. \sand{To add on the previous comment: while there have to be 16 or 64 entries addressable, note that having just 2, with 14 disabled/unavailable entries is also valid (and there are current RISC-V processors sold with such setups)}
...
\sand{In this paragraph I'm missing some information on UCs. You talk quite a bit about defining contracts for functions, but that leaves a question: if the UC has a pre- and postcondition, over what is it defined? The answer is the fetch-decode-execute cycle. Maybe at some explanation so that the concept of a UC is more tangible for readers.}
To then implement the \gls{uc} every function needs a definition of what it is supposed to do, which is done by defining contracts for every function as seen in \cref{lst:microsail_contract}. \sand{You don't "implement" the UC, you formalize, verify and apply it. ;) We define contracts for individual functions to aid in the verification of the UC, but it also enables verification of functional correctness of an indivudal piece, as well as making it easier to debug smaller parts of the verification. Technically you could just verify the UC without writing contracts for all the functions of the ISA spec, but it would be a horrible experience. 
...
When all the function's contracts are proven true, the prover can use these contracts to prove the \gls{uc}. \sand{I get what you mean here, but maybe rephrase it a bit: "these contracts can then be used to prove the UC". The term "prover" can mean either (1) the person proving these contracts, or (2) the tool, Katamaran/Rocq.}

% TODO also include the contract and verification for the chosen example function

\begin{lstlisting}[caption=Sail code example, label=lst:sail_code]
